...
async function runTests() {
  ...
  // Test Case 16: Jargon-ban on whyFacts and nextTarget strings
  // Confirms none of the plain-language fact/action strings exposed to the frontend
  // contain internal signal names, weight references, or banned words.
  {
    const BANNED_TERMS = [
      "signal", "repaymentReliability", "followThrough", "settlementInitiative",
      "consistency", "contribution", "weight", "dimension", "metric",
      "initiate", "settlement", "follow-through", "initiative",
    ];

    // Simulate realistic whyFacts and nextTarget strings as would be generated by insights.js
    const sampleWhyFacts = [
      { icon: "pending", sentiment: "warning", text: "2 pending payments still to be confirmed" },
      { icon: "check_circle", sentiment: "good", text: "85% of your payments were confirmed on time" },
      { icon: "payments", sentiment: "good", text: "You paid upfront for 45% of shared expenses" },
      { icon: "calendar_month", sentiment: "neutral", text: "Active in 4 months of group expenses" },
    ];

    const sampleActions = [
      "Pay back your 2 pending debts",
      "Keep your payments confirmed on time going forward",
      "Pay upfront for a few more shared group costs",
    ];

    const allStrings = [
      ...sampleWhyFacts.map(f => f.text),
      ...sampleActions,
    ];
    ...
    for (const str of allStrings) {
      for (const banned of BANNED_TERMS) {
        ...
        assert.ok(
          !lower.includes(bannedLower),
          `Jargon leak: "${banned}" found in whyFacts/nextTarget string: "${str}"`
        );
      }
    }

    // Confirm nextTarget is null when score is 100 (nothing to improve)
    const nextTargetAtMax = null; // score=100, no actions
    assert.strictEqual(nextTargetAtMax, null, "nextTarget must be null when score is 100");

    // Confirm whyFacts never includes a zero-pending entry
    const zeroFacts = sampleWhyFacts.filter(f => f.text.startsWith("0 pending"));
    assert.strictEqual(zeroFacts.length, 0, "whyFacts must not include a '0 pending' entry");

    console.log("✔ Test Case 16 Passed (whyFacts and nextTarget strings pass jargon-ban check)");
  }
