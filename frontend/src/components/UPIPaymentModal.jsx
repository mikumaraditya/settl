...
export default function UPIPaymentModal({ transaction, onClose, onPaymentInitiated }) {
  ...
  // Memoize upiUri to avoid recalculation on every render
  const upiUri = useMemo(() => {
    return `upi://pay?pa=${encodeURIComponent(to.upiId)}&pn=${encodeURIComponent(to.name)}&am=${amount}&cu=INR&tn=${encodeURIComponent('Settling on Settl')}`
  }, [to.upiId, to.name, amount])
  ...
  return (
    <div
      className="modal-overlay z-50 bg-black/50 backdrop-blur-sm flex items-end md:items-center justify-center p-4 animate-in fade-in duration-300"
    ...
    >
      <div className="upi-payment-modal glass-card relative w-full max-w-sm rounded-t-3xl md:rounded-3xl border border-white/10 flex flex-col gap-5 p-6 animate-in slide-in-from-bottom md:zoom-in-95 duration-250 max-h-[90vh] overflow-y-auto overflow-x-hidden custom-scrollbar !bg-[#0b1222] text-white">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-xs mt-0.5 text-slate-400 font-semibold">
              to <span className="font-bold text-white bg-white/5 px-2 py-0.5 rounded-md">{to.name}</span>
            </p>
          </div>
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-xl flex items-center justify-center bg-white/5 hover:bg-white/10 border border-white/5 transition-all cursor-pointer text-white shadow-sm"
          >
        ...
        {/* MOBILE VIEW (Screen sizes < md) - Pure CSS responsive rendering for zero resize listeners */}
        <div className="flex flex-col gap-4 md:hidden">
          {/* QR Code Container */}
          <div className="flex flex-col items-center gap-2.5">
            <div className="relative rounded-2xl p-4 bg-white shadow-lg flex items-center justify-center overflow-hidden laser-scanner border border-slate-200">
              <QRCodeSVG
                value={upiUri}
                size={140}
                bgColor="#ffffff"
            ...
            <p className="text-[10px] font-extrabold uppercase tracking-widest text-blue-400 flex items-center gap-1 mt-1">
        ...
        {/* DESKTOP VIEW (Screen sizes >= md) - Pure CSS responsive rendering */}
        <div className="hidden md:flex md:flex-col md:gap-4">
          ...
          {/* UPI ID + Copy */}
          <div className="flex items-center justify-between rounded-2xl px-4 py-3 gap-3 bg-white/5 border border-white/10 hover:border-white/20 transition-all">
