...
- **Google Gemini Integration**: On login, the dashboard queries the Google Gemini API (using the model `gemini-1.5-flash` with `gemini-2.0-flash` fallback).
...
| **Integrations** | Google Gemini API (`gemini-1.5-flash` / `gemini-2.0-flash`), Nodemailer / Gmail SMTP, UPI deep link protocols |
